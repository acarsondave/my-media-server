...
wget -qO- --method=PUT --body-data='{"status":"stopped"}' http://127.0.0.1:8000/v1/vpn/status > /dev/null

# Wait a moment for it to tear down
sleep 2

# Start the VPN connection
wget -qO- --method=PUT --body-data='{"status":"running"}' http://127.0.0.1:8000/v1/vpn/status > /dev/null
